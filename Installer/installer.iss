...
; v3.1.x: Win11 IExplorerCommand shell extension (Plan D).
; Win11's compact top-level context menu is populated by
; IExplorerCommand implementations. This registration adds
; "OCR 文字提取" to that menu.
;
; The COM in-proc server is implemented as a native comhost.dll
; (generated by EnableComHosting in the .NET 10 SDK) that lives
; in {app}\Plugins\ alongside the managed class DLL and a
; self-contained .NET 10 runtime. The OS COM activator loads
; the comhost directly (no mscoree mediation); the comhost
; then loads the .NET runtime and instantiates the managed
; OcrExplorerCommand class. This dual-DLL architecture is the
; official .NET 6+ COM server path and is the only way to
; make self-contained .NET COM in-proc servers work reliably.
;
...
; classic right-click menu (via the ContextMenuHandlers path
; which falls back to IContextMenu if IExplorerCommand isn't
; available). For Win11: shows in the top-level compact menu.
;
; The native comhost.dll is loaded by the COM activator when
; the user right-clicks an image file. The comhost then loads
; the .NET runtime and instantiates the managed
; OcrExplorerCommand class — all from the {app}\Plugins\
; directory where the self-contained publish places both the
; comhost, the managed assembly, and the .NET runtime DLLs.
;
; Gated by the same `ocrverb` task as the IContextMenu fallback
; entries so the user opts in / out once for OCR right-click
; integration in general. Removing the task also unregisters
; the COM class and the ContextMenuHandlers association.
...
Root: HKCU; Subkey: "Software\Classes\CLSID\{#OcrShellExtClsid}\InprocServer32"; ValueType: string; ValueName: ""; ValueData: "{app}\Plugins\ApertureNeo.Plugins.Ocr.ShellExt.comhost.dll"; Flags: uninsdeletekey; Tasks: ocrverb
...
; file association.
; (2) Link the COM class to the image file class. This is the
; standard shellex\ContextMenuHandlers path that the shell uses
; to find IExplorerCommand extensions for the modern Win11
; compact menu. The value data is the CLSID; the name is a
; friendly label. We register under SystemFileAssociations\image
; so the verb shows for every image file (jpg, png, bmp, etc.)
; without needing a per-extension registry entry.
; NOTE: the previous registration used shellex\ExplorerCommand
; which is NOT a standard shellex subkey — Windows ignores it
; silently. Plan D fixes to ContextMenuHandlers and also points
; InprocServer32 at the native comhost.dll instead of the
; managed DLL, fixing both the .NET runtime activation issue
; (missing runtimeconfig.json) and the registration path issue.
Root: HKCU; Subkey: "Software\Classes\SystemFileAssociations\image\shellex\ContextMenuHandlers\ApertureNeoOcr"; ValueType: string; ValueName: ""; ValueData: "{#OcrShellExtClsid}"; Flags: uninsdeletekey; Tasks: ocrverb
...
; v3.1.x (Plan C fallback): classic IContextMenu verbs. The
; IExplorerCommand verb above targets Win11's top-level
; (compact) context menu, but COM activation in self-contained
; .NET 10 publishes has a history of subtle failures (missing
; deps.json, .NET runtime location, etc.). The IContextMenu
; path is the well-established Win32 shell verb path that
; always works on Win10 + "Show more options" on Win11. We
; register both so the user always has the verb available —
; IExplorerCommand is the preferred Win11 path, IContextMenu
; is the safety net. The same `ocrverb` task controls both.
;
...
begin
  ...
  begin
    ...
    RegDeleteKeyIncludingSubkeys(HKCU, 'Software\Classes\SystemFileAssociations\image\shellex\ContextMenuHandlers\ApertureNeoOcr');
